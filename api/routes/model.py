...
def create():
    ...
    if model_name == "" or model_name is None:
        return error_response(
            "O campo nome do modelo é obrigatório", status.HTTP_400_BAD_REQUEST
        )

    model_description = request.form.get("description")
    if model_description == "" or model_description is None:
        return error_response(
            "O campo descrição é obrigatório", status.HTTP_400_BAD_REQUEST
        )
    ...
    if filename == "":
        return error_response(
            "O arquivo contendo o modelo é obrigatório", status.HTTP_400_BAD_REQUEST
        )
...
def delete(id: int):
    ...
    return error_response("Modelo não encontrado", status.HTTP_404_NOT_FOUND)
